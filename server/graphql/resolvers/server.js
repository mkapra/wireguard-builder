...
module.exports = {
    ...
    Query: {
        servers: async(_, { name }, { dataSources: { db } }) => {
            ...
            try {
                ...
                return servers.filter(server => { return re.test(server.name); });
            } catch (e) {
                throw new UserInputError(`Name is not a regex: ${e}`);
            }
        },
        server: async(_, { id }, { dataSources: { db } }) => {
            return db.getServer(id);
        }
    },
    Mutation: {
        createServer: async(_, { newServer: { name, description, forward_interface, ip_address, keypair, vpn_network } }, { dataSources: { db } }) => {
            ...
            if (keypair_used) {
                throw new UserInputError(`Keypair with id ${keypair} is already used by server ${keypair_used.name}`);
            }

            // validate vpn_network exists
            const vpn_network_exists = await db.getVpnNetworkById(vpn_network);
            if (!vpn_network_exists) {
                throw new UserInputError(`Vpn network with id ${vpn_network} does not exist`);
            }
            ...
            const ip_address_in_range = validator.isInRange(ip_address, vpn_ip, vpn_network_range);
            ...
            console.log("Created: " + createdServer);
